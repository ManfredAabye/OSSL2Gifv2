...
requirements.txt:
Listet die benötigten Python-Pakete auf, die mit pip installiert werden können (z.B. Pillow, ttkbootstrap). Keine ausführbaren Skripte oder Funktionen.
...
threading.py:
Enthält Funktionen oder Klassen, die für die Ausführung von Hintergrundthreads zuständig sind, um die GUI reaktionsfähig zu halten, während zeitintensive Bildverarbeitungsaufgaben ausgeführt werden.

tooltip.py:
Enthält die Klasse für Tooltips, die in der GUI verwendet werden, um Hilfetexte anzuzeigen.

translations.py:
Beinhaltet die Übersetzungen für die mehrsprachige Benutzeroberfläche (Texte für Deutsch, Englisch, Französisch, Spanisch, Italienisch, Russisch, Niederländisch, Schwedisch, Polnisch, Portugiesisch).
